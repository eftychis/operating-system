...
int P6_dfat(int argc, char* argv[])		// list FAT table
{
	...
	switch (argc)
	{
		...
		case 2:						//	2	fat <1 to 2>		output fat <#> table
		{
			...
			if ((index == 1) || (index ==2))
			{
				sprintf_s(buf, 32, "Disk Image: FAT %d", index);
...
int P6_dumpSector(int argc, char* argv[])	// dump RAM disk sector
{
	...
	sprintf_s(temp, 32, "Sector %d:", sector);
...
void printFatEntries(unsigned char* FAT, int start, int end)
{
   ...
   for (i=start; i<nn; i++)
   {
      ...
      else if (fatvalue == FAT_EOC) sprintf_s(tbuf, 15, "  EOC"); // The EOC marker
      else if (fatvalue == FAT_BAD) sprintf_s(tbuf, 15, "  BAD"); // The BAD cluster marker
	...
	switch (test)
	{
		...
		case 2:					// open files
		{
			...
			for (i=0; i<numWords; i++)
			{
				...
				strncat_s(rBuf, 127, buf, strlen(text[i]));
			}
			printf("\n  %s", rBuf);
		...
		case 3:
		{
			...
			printf("\nRunning Test 3...");
			sprintf_s(buf2, 31, "file%d.txt", test2);
			for (i=0; i<numWords; i++)
			{
				sprintf_s(buf, 31, "file%d.txt", i);
				// open word file
